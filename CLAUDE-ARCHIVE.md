# CLAUDE.md archive: 2026-08-01 through 2026-08-17

Relocated wholesale (verbatim, nothing cut) from the main `CLAUDE.md` on 2026-09-19, purely to
keep the file auto-loaded into every session's context smaller. This covers the project's first
~2.5 weeks — the Apps-Script-to-Supabase migration, the original admin-dashboard/nav redesigns
(since superseded by later rounds still in `CLAUDE.md`), and roughly 90 session-by-session
bugfix/feature entries from that period.

**When to open this file**: `CLAUDE.md` itself still documents every *standing* convention/footgun
this period established (see "Known duplicated logic (footguns)" near its top) — you shouldn't
need this file for day-to-day work. Open it when: a current bug's symptoms match something that
sounds like it might have happened before during this period; `CLAUDE.md` references a fact from
"the migration" or an early admin-redesign round and you want the full original reasoning, not
just the summary; or you're specifically working on GAS/Supabase migration history, the original
`project_access`/RLS build-out, or the pre-2026-08-21 admin nav/dashboard designs.

Entries are otherwise unchanged from their original form — same dating convention, same
"Verified"/"Not verified" language (see `CLAUDE.md`'s own "Standing verification posture" note,
which applies here too), same session-handoff sections interleaved chronologically.
...
- Session handoff (2026-08-04, later session)
...
- Session handoff (2026-08-05, end of session)
- Survey editor: participant-view "Preview" feature (2026-08-05)
- Session handoff (2026-08-05, later session — survey preview feature)
...
- Session handoff (2026-08-06, dark mode)
- Dark mode for participants (feed + survey), admin-controlled, synced into previews (2026-08-06)
- Session handoff (2026-08-06, dark mode for participants)
...
- Session handoff (2026-08-06, simulator/CSV/abuse-guards/attention-checks)
...
Recap of the Users/access-control rework, username/role polish, the self-lockout incident, the
nav/UX batch, and "default feed" removal — full detail in this file's own dated sections above.
Plan file: `~/.claude/plans/patient-guarding-lovelace.md`. Superseded by the next handoff below.
Its flagged item (`project_access` RLS never exercised against a real restricted session) was
carried forward and resolved 2026-08-05 — see "`project_access` RLS finally verified live" further
down.
...
Recap of a "make the admin dashboard more professional" pass (browser-dialog replacement +
loading/empty states + unsaved-changes guards; error boundaries + modal accessibility + SVG icons;
button consistency + image compression), plus three real bugs the user reported live and that got
fixed the same session (SVG icon misalignment, unstyled/floating dialogs, a `feed_surveys`
foreign-key violation on a real study). Full detail in the dated sections above. Plan file:
`~/.claude/plans/vigilant-mending-osprey.md`. `project_access` RLS (still not verified against a
real restricted session, two sessions running at this point) was resolved 2026-08-05 — see
"`project_access` RLS finally verified live" below.
...
Recap of a long overnight session (RLS audit, the participants analysis hub, an Amazon
randomization fix, data-quality flags, and the full staging-environment build-out — the "live
dashboard"/"suspicious participants" asks had no record anywhere when investigated, so both were
built from scratch as a reasonable interpretation, flagged in case that reading was wrong). Plan
file: `~/.claude/plans/quiet-bridging-noether.md`. Staging (`staging.studyfeed.org`) became usable
for a real admin click-through for the first time this session — a fact restated (and relied on)
throughout the rest of this file from here on.
...
Recap of the survey-editor redesign (bug fix + 5 restyling stages) — full detail in the dated
section above. Plan file: `~/.claude/plans/synthetic-sprouting-crescent.md`. Two ideas were raised
in this session's own "what next" discussion but not built, and were never picked up again anywhere
later in this file — still open if ever revisited: an admin audit/activity log (no action-history
UI exists anywhere in this app, despite several incidents this file documents that one would have
sped up), and Amazon reviews never rendering their own configured images (`ReviewCard` hardcodes
`data-has-image="0"`, confirmed via grep, no `<img>` anywhere in its markup).
...
Survey preview feature shipped (twice bugfixed) — full narrative in the dated section above. Plan
file: `~/.claude/plans/purrfect-wiggling-quill.md`. The admin audit/activity-log and Amazon
review-image gaps from the prior handoff were still untouched as of this session too (see above).

**Debugging lesson worth keeping**: fabricated test data can pass every check while a real survey
still fails, if the real survey has a data shape nobody thought to fabricate (here: two
post_reminder questions sharing one bare `post_id` across different feeds). When isolated
reproduction keeps passing but a user's real report keeps failing, pulling the actual real data
directly (read-only, anon-key, no login needed) is far more productive than more synthetic guesses.
...
**Superseded by "Session handoff (2026-08-06, end of session)" further down** — three more real
bugs were found and fixed after this note was written, and `production` has since caught up to
`main`. Left in place as history; no unique content beyond what the later handoff restates.
...
Recap of a long session: survey editor/preview polish, a new Feed Preview feature, a Feed
Participants tab redesign, and a multi-round "realism" feature (engagement counts / pacing / real
surroundings) that surfaced three real, distinct bugs via direct user testing — each root-caused
and fixed the same session. Full detail lives in the dated sections above, from "Realism
improvements..." through this one; this is an index, not a re-explanation. No plan file exists for
this session (nothing went through `EnterPlanMode`) — this CLAUDE.md section is the complete record.
`main`/`production` were confirmed in sync at session end, so everything shipped went live on
`studyfeed.org` directly, with no staging buffer to catch anything first.

**Debugging playbook worth keeping, distilled from how the three bugs were actually found** — if a
future "doesn't work" report on a flag/feature comes in: (1) don't trust "the admin UI shows it as
on" as proof of anything past the write path — query the actual stored value directly
(`supabase db query --linked`, relink between production/staging as needed, relink back to
production when done); (2) don't trust "the component renders correctly with fabricated props" as
proof it works live — that skips the real fetch → normalize → render pipeline, which is exactly
where one of these bugs hid (each `App-*.jsx`'s own `normalizeFlags()` silently dropping new flag
keys); (3) when checking whether an element is actually visible, measure `getBoundingClientRect()` +
computed `display`, not just presence of the right CSS class — `document.querySelector` returns the
*first* match, and this codebase has more than one place independently rendering something matching
the same class names (e.g. `.rail-left`/`.rail-right`: the pre-load skeleton, `Feed`'s own rails,
and `PageWithRails`'s rails); (4) fetch the actual deployed JS bundle directly (`curl` the hashed
chunk, `grep` for the symbol) before assuming a fix isn't live — deploys are fast and confirmable
via the GitHub Actions API without guessing.
...
Recap of the admin-dashboard dark-mode work — full detail in the dated section above. Plan file:
`~/.claude/plans/twilight-toggling-hopper.md`. Everything was verified via faked-local-session
component mounts, not a real logged-in click-through — the several "Dark mode, round N" sections
further down closed most of that gap by finding real bugs the same way live testing usually does
in this file.
...
Recap of participant-facing dark mode (feed + survey) — full detail in the dated section above.
Plan file: `~/.claude/plans/valiant-stirring-kernighan.md`. Verified via component mounts with
fabricated data only; later "Dark mode, round N" sections found and fixed several real bugs this
pass missed, via actual live testing.
...
Recap of four arcs: the survey response simulator, CSV filename/column-prefix cleanup, submission
abuse guards (rate limiting + duplicate-prevention + an already-completed guard), and attention-
check questions. Full detail in the dated sections above. Plan file:
`~/.claude/plans/watchful-guarding-shannon.md`.

**Still open, never followed up on anywhere later in this file**: 5 pre-existing duplicate
`(feed_id, prolific_pid)` pairs in `proj_7` — flagged to the user once, possibly a double Prolific
payout on a real study, worth revisiting.

**Explicitly deprioritized by the user, not forgotten — don't re-pitch unless asked**: paste-
detection + tab-away tracking on survey text questions (would need to ship *before* real data
collection to be useful at all), and near-duplicate free-text-answer detection (the user doesn't
use many open-ended questions, so low value for this study).
...
Recap of a long session: two Survey Participants analysis-hub fixes, a screenshot-driven redesign of
Facebook's "Realistic surroundings" rails (which surfaced three real bugs), a real CORS/CloudFront
caching bug that had silently broken avatar photos everywhere, a fourth round of dark-mode fixes, an
urgent live-production bug (participants unable to submit surveys), a script to fix oversized
per-post images (written, later run — see below), and a real bug + redesign of the "Realistic
engagement counts" reaction icons. Pure frontend changes throughout — no migrations. Given the
survey-submission bug was live and urgent, this session flagged prioritizing `production` promotion
over the normal cadence.

**Still open as of the 2026-08-11 handoff below, never resolved since**: whether any participants
got stuck/lost data before the survey-submission fix shipped (worth a spot-check against real
`survey_responses` rows for "Study 1 - Main"), and the reaction-pill overlap math being slightly off
(~4px actual vs ~2px intended, from a compounding inline `marginLeft` on top of the CSS
`margin-right`).
...
Recap of a long session covering: reaction-icon/pill polish, engagement-count randomization,
rail/top-bar click-interception fixes, the oversized-image resize script actually running (53 files,
see above), two explanatory-only conversations, Study-overview page-number/filter sync fixes, and
the feed+survey merged CSV fixes (including a real data-loss bug and a new per-feed CSV name field).
Full detail in the dated sections above.

**Resolved from the 2026-08-08 handoff's open list**: the oversized-image resize script — done, 53
files fixed and verified live against the CDN. The other two items from that list (participant
data-loss spot-check, reaction-pill overlap math) were carried forward unresolved and never picked
up again anywhere later in this file — still open if ever revisited.

**Standing preference, don't re-pitch unless asked**: AWS access stays manual/per-session by
explicit user choice — a persistent scoped-IAM-user alternative was offered and declined.
