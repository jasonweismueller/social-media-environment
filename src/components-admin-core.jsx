...
export function AdminDashboard({
  ...
  // ---- feed flags state (per project+feed)
  const [feedFlags, setFeedFlags] = useState({}); // key: "pid::fid" -> { random_time, loaded, loading, saving }
 ...
 const loadFlagsFor = async (fid, { force = false } = {}) => {
  ...
  // always fetch stats & flags for the currently selected feed (so titles/buttons are correct)
  useEffect(() => {
  ...
  const loadFeeds = useCallback(async () => {
    ...
    try {
      ...
      if (chosen) {
        ...
        // prefetch flags for the visible/important feed
  ...
  useEffect(() => {
    ...
    // clear per-project feed stats & flags cache when switching projects
    setFeedStats({});
    setFeedFlags({});
...
const anySaving = !!(ff.saving || ff.savingAv || ff.savingNm);
  ...
  // prevent a race if any toggle is already saving
