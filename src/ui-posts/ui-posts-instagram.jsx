            ...
            <button
              ...
              onClick={() => {
                ...
                if (item.action !== "cancel") {
                  onPick?.("report_misininformation_click", { id });
                }
              }}
              style={{
                display: "block",
...
export function PostCard({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const list = await getAvatarPool(inferredAuthorType);
        if (cancelled) return;
  ...
  const timeLabel = useMemo(() => {
    ...
    if (!time && randTimesOn) {
      ...
      return (picked || "2 hours ago").toLowerCase();
    }
  ...
  const videoAutoplayMuted = !!post.videoAutoplayMuted;
  const videoShowControls = !!post.videoShowControls;
  const videoLoop = !!post.videoLoop;
  const shouldAutoplayVideo = videoAutoplayMuted;

  const autoplayVideoRef = useInViewAutoplay(0.6, {
    startMuted: videoAutoplayMuted,
    unmuteOnFirstGesture: videoAutoplayMuted
  });
  const manualVideoRef = useRef(null);
  const videoRef = shouldAutoplayVideo ? autoplayVideoRef : manualVideoRef;
  ...
  }, [isMobile, hoverTargetEl, refFromTracker]);
