...
import { Feed as XFeed } from "./ui-posts";
// Direct file import (not the per-app `./ui-posts` barrel) — this app is
// unambiguously X, and `PageWithRails` below needs the exact same
// seeded-contacts builder + left-rail label lists `Feed`'s own rails use
// (ui-posts-x.jsx), so the two "realistic surroundings" renderers (this
// file's `PageWithRails`, the one actually wrapping the live participant
// feed, and `Feed`'s own rails, only reachable when `Feed` is mounted
// standalone e.g. the admin's Feed Preview) can't drift apart. Mirrors the
// identical Facebook/Instagram precedent for this exact import shape.
import {
  buildRailContacts,
  LEFT_RAIL_NAV_ITEMS,
  LEFT_RAIL_ICONS,
  TRENDING_TOPICS,
} from "./ui-posts/ui-posts-x";
import {
  ParticipantOverlay,
  ThankYouOverlay,
  RouteAwareTopbar,
...
/* =========================================================================
   X (Twitter) app with survey support
   Ported from the Facebook survey-enabled app while adapting to X's own
   visual/interaction model (reply/repost/like/view/bookmark instead of
   FB's reactions, no left-rail "shortcuts" concept, right rail is
   Trending + Who to follow instead of Contacts).
   ========================================================================= */
...
if (typeof document !== "undefined") {
  document.body.classList.toggle("x-mode", MODE === "x");
}
  ...
  return Promise.all(
    Array.from(uniqueTargets.values()).map(
      async ({ feedId, postId, applyFeedRandomization }) => {
        ...
        // (ui-posts-x.jsx) computes a randomized pool image itself,
...
function PageWithRails({ children, flags, runSeed, app, projectId, feedId }) {
  ...
  // This component — not `Feed`'s own internal rails (ui-posts-x.jsx) — is
  // what actually wraps the live participant feed (see its call site
  ...
  // Real X has no "shortcuts"/groups concept the way Facebook's left rail
  // does — just the flat nav list — so this is simpler than the FB/IG
  // versions: one nav block on the left, Trending + Who to follow on the
  // right, no separate shortcuts sub-section or shortcuts-count sizing.
  const realisticOn = !!flags?.realistic_surroundings;
  const [contacts, setContacts] = useState([]);

  // The real-content rows are much shorter than the ghost `RailBox`/
  // `RailList` blocks `rightCount` above was tuned for (a compact
  ...
  // ghost version did. Tuned to the real rows' actual ~52px height,
  // recomputed on the same resize listener as the ghost version.
  const [realRightCount, setRealRightCount] = useState(8);

  useEffect(() => {
    const compute = () => {
      const railH = (window.innerHeight || 900) - 30;
      const REAL_ROW_H = 52;
      const TRENDING_BLOCK_H = TRENDING_TOPICS.length * REAL_ROW_H + 90;
      const WHO_TO_FOLLOW_HEADER_H = 90;
      const rightN = Math.max(
        3,
        Math.min(Math.floor((railH - TRENDING_BLOCK_H - WHO_TO_FOLLOW_HEADER_H) / REAL_ROW_H), 20)
      );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // suggestions only get a real avatar photo when it's on; off, they
      // fall back to the existing blank-circle placeholder (no pool fetch
      // at all, so turning this off also skips the extra network cost).
    ...
    >
      {realisticOn ? (
        <aside className="rail rail-left rail--content" aria-hidden="true">
          ...
          <div className="rail-real-post-pill" aria-hidden="true">
            Post
          </div>
        </aside>
      ) : (
...
export default function App() {
  ...
  // feed_interlude support — see App-facebook.jsx's identical block for the
  // full rationale (this file mirrors that one, same "near-duplicate
  // App-*.jsx" convention as everything else here).
  ...
  // See App-facebook.jsx's identical block for the full rationale.
  ...
  const canShowFeed =
    hasEntered &&
    feedPhase === "ready" &&
    // See App-facebook.jsx's identical comment.
  ...
  const activeLoadingOverlay =
    ...
    // the feed_interlude counterpart — see App-facebook.jsx's identical
                      ...
                      {canShowFeed ? (
                        <XFeed
                          ...
                          onSubmit={async () => {
                            ...
                            // See App-facebook.jsx's identical try/catch/
                            // finally for the full rationale — without it,
                            // any thrown exception here (not just a
                            // sendToSheet failure) left the participant
                            // stuck on the interlude feed under a permanent
                            // spinner with no way back and no visible error.
                            try {
                              const ENTER_FRAC = Number.isFinite(Number(VIEWPORT_ENTER_FRACTION))
                                ? clamp(Number(VIEWPORT_ENTER_FRACTION), 0, 1)
                                : 0.5;
