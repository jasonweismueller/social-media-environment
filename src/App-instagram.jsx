      ...
      async ({ feedId, postId, applyFeedRandomization }) => {
        ...
        if (flags.randomize_avatars ?? flags.randomize_avatar) {
          try {
            ...
            // Must match PostCard's own render-time pool choice exactly
            // (getAvatarPoolForPost, ui-posts-instagram.jsx) — preloading
            // the plain pool for a misinformation-flagged post would warm
            // the wrong image, leaving the real pick to load cold.
...
export default function App() {
  ...
  const ensureSurveyLoaded = useCallback(async () => {
    ...
    try {
      ...
      const normalizedSurveyBase = surveyDef
      ...
      // survey_only delivery doesn't visit a real feed page — activeFeedId
      // here always resolves to the survey's first linked feed regardless
      // of which experiment group the participant landed in (see the
      // "infinite loading loop" fix above), so feed_overrides/
      // visible_in_feeds on ordinary questions can't vary by group without
      ...
      if (normalizedSurvey) {
        await preloadSurveyPostReminders({
          ...
          fallbackFeedId: isSurveyOnlyMode
  ...
  // Ported from App-facebook.jsx — see its own comment for the full
  // rationale.
  ...
  const canShowFeed =
    hasEntered &&
    feedPhase === "ready" &&
    // See App-facebook.jsx's identical comment — a feed_interlude's own
    // Feed branch reuses this gate too.
          ...
          <Route
            ...
            element={
              shouldShowSurvey ? (
                ...
                <InstagramSurroundings flags={flags} runSeed={runSeed} app={APP} projectId={projectId} feedId={activeFeedId}>
                  <div
                    style={{
                      position: "relative",
