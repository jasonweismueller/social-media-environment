...
/* ============================ Feed flags (random times) ============================ */
/**
 * URL override (for dev): ?randomtime=1 (or randomtimes / random_time / rt)
 * - 1/true/on => enabled, 0/false/off => disabled
 * - takes precedence over stored/remote flags
 */
function __urlOverrideRandomTimes() {
  try {
    const sp = getCombinedSearchParams();
    const v = sp.get("randomtime") || sp.get("randomtimes") || sp.get("random_time") || sp.get("rt");
...
const FEED_FLAGS_KEY = (projectId, feedId) =>
  `${APP}::${projectId || "global"}::${feedId || ""}::feed_flags_v1`;

export function readFeedFlagsLocal(projectId = getProjectId(), feedId = getFeedIdFromUrl()) {
  try {
    const raw = localStorage.getItem(FEED_FLAGS_KEY(projectId, feedId));
    return raw ? JSON.parse(raw) : {};
  } catch { return {}; }
}
export function writeFeedFlagsLocal(projectId = getProjectId(), feedId = getFeedIdFromUrl(), flags = {}) {
  try {
    localStorage.setItem(FEED_FLAGS_KEY(projectId, feedId), JSON.stringify(flags || {}));
  } catch {}
}

/** GET flags for a feed (e.g., { random_time: true }) */
export async function getFeedFlagsFromBackend(feedId) {
  try {
    const url = `${GS_ENDPOINT}?path=feed_flags&app=${APP}${qProject()}${feedId ? `&feed_id=${encodeURIComponent(feedId)}` : ""}&_ts=${Date.now()}`;
    const data = await getJsonWithRetry(url, { method: "GET", mode: "cors", cache: "no-store" }, { retries: 1, timeoutMs: 8000 });
    return (data && typeof data === "object") ? data : {};
  } catch (e) {
    console.warn("getFeedFlagsFromBackend failed:", e);
    return {};
  }
}

/** POST flags for a feed (admin only). Example: setFeedFlagsOnBackend(fid, { random_time: true }) */
export async function setFeedFlagsOnBackend(feedId, flags = {}) {
  const admin_token = getAdminToken();
  if (!admin_token) return { ok: false, err: "admin auth required" };
  try {
    const res = await fetch(GS_ENDPOINT, {
    ...
    if (!res.ok || json.ok === false) return { ok: false, err: json?.err || `HTTP ${res.status}` };
    // cache locally on success
    const pid = getProjectId();
    writeFeedFlagsLocal(pid, feedId, flags);
    return { ok: true };
  } catch (e) {
    return { ok: false, err: String(e?.message || e) };
  }
...
/**
 * Resolve the Random Times flag for the current feed:
 * - URL override (dev) wins
 * - else local cache
 * - else backend (and cache result)
 * Returns a boolean.
 */
export async function resolveRandomTimesFlag(feedId = getFeedIdFromUrl()) {
  // 1) URL override
  const ov = __urlOverrideRandomTimes();
  if (ov != null) return !!ov;

  // 2) Local cache
  const pid = getProjectId();
  const local = readFeedFlagsLocal(pid, feedId);
  if (typeof local.random_time === "boolean") return local.random_time;

  // 3) Backend
  const flags = await getFeedFlagsFromBackend(feedId);
  if (flags && typeof flags.random_time === "boolean") {
    writeFeedFlagsLocal(pid, feedId, flags);
    return flags.random_time;
  }
  return false;
}

/** Convenience for admin editor toggles (updates local cache only) */
export function setRandomTimesLocal(enabled, feedId = getFeedIdFromUrl()) {
  const pid = getProjectId();
  const next = { ...(readFeedFlagsLocal(pid, feedId) || {}), random_time: !!enabled };
  writeFeedFlagsLocal(pid, feedId, next);
}
