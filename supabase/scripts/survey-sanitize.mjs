// Verbatim copy of src/utils/utils-survey.js, for the Phase 3 migration script.
// Copied (not re-ported) so this Node script uses the exact same normalization
// logic as the frontend and the Phase 2 Edge Function, with zero transcription
// risk. Only change from the source file: uid() is inlined below instead of
// imported from utils-core.js (which has browser-only `window` references
// elsewhere in that file that would break a plain Node import).
// Keep in sync by hand with src/utils/utils-survey.js if that file changes.
