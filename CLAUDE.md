...
`visible_to_group_ids: []` (empty = shown to everyone), server-coordinated round-robin
assignment (`assign_experiment_group` Apps Script action, `LockService`-guarded, idempotent per
`session_id`). Full design rationale and file-by-file breakdown:
`~/.claude/plans/dapper-growing-emerson.md`.

**Status: frontend fully implemented and committed** (commit `001f463 add survey group
randomizer` and the plan file). **Backend Code.gs changes were handed to the user as a
copy-paste-ready full file, not applied by Claude** (can't touch their Apps Script directly) —
unverified whether they've pasted + redeployed it yet. If experiment groups don't seem to work
in a future session, check that first before debugging frontend code.

## Build/dev notes

- `npm run dev` — Vite dev server.
- `npm run build` — **currently broken in this environment**: `@rollup/rollup-darwin-arm64`
  native binary is blocked by macOS code-signing policy (unrelated to any app code). Workaround
  used all session: `./node_modules/.bin/esbuild <file> --loader=jsx --bundle=false --format=esm
  --outfile=/dev/null` per changed file, as a syntax-only check. Fix the real build with
  `rm -rf node_modules package-lock.json && npm i` if a real bundle build is ever needed.
